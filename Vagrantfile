...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    bash -c 'echo "export TZ=America/Sao_Paulo" > /etc/profile.d/tz.sh'
...
EOF
  ...
  config.vm.define "master", primary: true do |node|
    ...
    node.vm.provision "shell", inline: <<-SHELL
      yum -y install git net-tools bind-utils iptables-services bridge-utils bash-completion kexec-tools sos psacct wget git zile kexec-tools openssl-devel httpd-tools  NetworkManager python-cryptography python2-pip python-devel  python-passlib java-1.8.0-openjdk-headless "@Development Tools"
      ...
      yum install net-tools tcpdump nmap bind-tools -y 
